<template>
  <div>
    <Modal :title="L('CreateNewTimePeriod')" :value="value" @on-ok="save" @on-visible-change="visibleChange" :mask-closable="false" :transfer="false">
      <Form ref="timePeriodForm" label-position="top" :rules="TimePeriodRule" :model="timePeriod">
        <Row :gutter="16">
          <i-col span=12>
            <FormItem :label="L('Start')" prop="start">
              <TimePicker type="time" placeholder="Select time" v-model="timePeriod.start" style="100%" format="HH:mm" :steps="[1, 5]"></TimePicker>
            </FormItem>
          </i-col>

          <i-col span=12>
            <FormItem :label="L('End')" prop="end">
              <TimePicker type="time" placeholder="Select time" v-model="timePeriod.end" style="100%" format="HH:mm" :steps="[1, 5]"></TimePicker>
            </FormItem>
          </i-col>
        </Row>
        <FormItem>
...
@Component
export default class CreateTimePeriod extends AbpBase {
