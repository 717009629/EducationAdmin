    ...
    <Modal :title="L('CreateNewTenant')" :value="value" @on-ok="save" @on-visible-change="visibleChange">
      <Form ref="tenantForm" label-position="top" :rules="tenantRule" :model="tenant">
        <FormItem :label="L('TenancyName')" prop="tenancyName">
          <Input v-model="tenant.tenancyName" :maxlength="32" :minlength="2" />
        </FormItem>
        ...
        <FormItem :label="L('DatabaseConnectionString')" prop="databaseConnectionString">
          <Input v-model="tenant.databaseConnectionString" :maxlength="1024" />
        </FormItem>
        <FormItem :label="L('AdminEmailAddress')" prop="adminEmailAddress">
          <Input v-model="tenant.adminEmailAddress" type="email" :maxlength="256" />
        </FormItem>
        <FormItem>
          <Checkbox v-model="tenant.isActive">{{L('IsActive')}}</Checkbox>
        </FormItem>
          <p>{{L("DefaultPasswordIs",undefined,'123qwe')}}</p>
      </Form>
      <div slot="footer">
