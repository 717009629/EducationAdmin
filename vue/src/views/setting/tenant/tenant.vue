    ...
    <Card dis-hover>
      <div class="page-body">
        <Form ref="queryForm" :label-width="100" label-position="left" inline>
          ...
          <Row>
            <Button @click="create" icon="android-add" type="primary" size="large" v-if="hasPermission('Pages.Tenants.Create')">{{L('Add')}}</Button>
            <Button icon="ios-search" type="primary" size="large" @click="getpage" class="toolbar-btn">{{L('Find')}}</Button>
          </Row>
        </Form>
        <div class="margin-top-10">
          <Table :loading="loading" :columns="columns" :no-data-text="L('NoDatas')" border :data="list">
            <template slot-scope="{ row }" slot="action" v-if="hasPermission('Pages.Tenants.Edit')||hasPermission('Pages.Tenants.Delete')">
              <Button v-if="hasPermission('Pages.Tenants.Edit')" type="primary" size="small" @click="edit(row)" style="margin-right:5px">{{L('Edit')}}</Button>
              <Button v-if="hasPermission('Pages.Tenants.Delete')" type="primary" size="small" @click="remove(row)" style="margin-right:5px">{{L('Delete')}}</Button>
            </template>
...
export default class Tenants extends AbpBase {
  edit(row) {
    this.$store.commit("tenant/edit", row);
